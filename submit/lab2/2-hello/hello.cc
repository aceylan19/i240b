...
{
  bool inWord = false;
  const int n = std::strlen(in)+1; // becacuse there is a "/0" at the end of the string to sse the terminator we added +1
  for (int i = 0; i < n; i++) {
...
main(int argc, const char *argv[])
{
  if (argc == 1) {
    ...
    for (int i = 1; i < argc; i++) { // i should not be equal to argc value.
